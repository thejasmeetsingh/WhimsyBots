...
class Bot(BaseModel):
    ...
    Validation:
        - Either interval_mins OR cron_expression must be provided (not both)
        - Cron expressions are validated for correctness
    """
    ...
    interval_mins = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="Runs every X minutes (minimum: 5)",
    ...
    cron_expression = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Scheduling in standard cron format (alternative to interval)",
...
class Message(BaseModel):
    ...
    intent = models.CharField(max_length=1, choices=MessageIntentType.get_values(), null=True, blank=True)
    content = MartorField()
...
class Log(BaseModel):
    ...
    description = models.TextField(null=True, blank=True, help_text="Error message or execution details")
